...
export default function Kelas() {
  ...
  const handleSaveClick = () => {
    ...
    if (!kelasValue.trim()) {
        ...
        setIsKelasValid(true);
    }

    if (!jurusanValue.trim()) {
        setIsJurusanValid(false);
        isValid = false;
    } else {
        setIsJurusanValid(true);
    }

    // Fokus pada input yang pertama kali ditemukan kosong
    if (!isValid) {
        if (!kelasValue.trim()) {
            document.getElementById('kelasInput').focus();
        } else if (!jurusanValue.trim()) {
  ...
  const handleConfirmDelete = () => {
    ...
    setTableData(updatedData);
    localStorage.setItem("tableDataKelas", JSON.stringify(updatedData)); // Update localStorage
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-1/3 p-4 lg:p-6">
            ...
            <input
                ...
                className={`w-full p-2 border rounded text-sm sm:text-base mb-2 ${isKelasValid ? 'border-gray-300' : 'border-red-500'}`}
                placeholder="Kelas..."
              />
              <h2 className="text-sm mt-4 mb-2 sm:text-sm font-bold"> Jurusan</h2>
              <select
                id="jurusanInput"
                value={jurusanValue}
                onChange={handleJurusanChange}
                className={`w-full p-2 border rounded text-sm sm:text-base mb-2 ${isJurusanValid ? 'border-gray-300' : 'border-red-500'}`}
              >
